...
export class Trail extends THREE.Object3D {
  ...
  reset() {
    ...
    // let colors = [0xaaaaaa, 0xbbbbbb, 0xcccccc, 0xdddddd, 0xeeeeee, 0xFFFFFF]
  ...
  update() {
    ...
    this.position.y = this.position.y + (targetPosition - this.position.y) * 0.1
    this.position.z -= 0.5
    ...
    for(let i = 0; i < this.positionHistory.length ; i++) {
      if(this.tail) {
        this.tail.geometry.vertices[i].set(this.position.x, this.positionHistory[i], this.position.z + (i * 1.0))
